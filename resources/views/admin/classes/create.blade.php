    ...
    <div class="py-12">
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-xl sm:rounded-lg p-8">
                <h1 class="text-3xl font-bold text-gray-900 dark:text-white mb-8">Add New Class</h1>
                ...
                <form action="{{ route('classes.store') }}" method="POST">
                    ...
                    <div class="grid grid-cols-1 gap-6">
                        <div>
                            ...
                            <input type="text" name="name" value="{{ old('name') }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 dark:bg-gray-900 dark:text-white dark:border-gray-600" required>
                            @error('name') <span class="text-red-500 text-sm">{{ $message }}</span> @enderror
                        </div>

                        <div>
                            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">Description</label>
                            <textarea name="description" rows="3" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 dark:bg-gray-900 dark:text-white dark:border-gray-600">{{ old('description') }}</textarea>
                        ...
                        <div>
                            ...
                            <select name="teachers[]" multiple class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 dark:bg-gray-900 dark:text-white dark:border-gray-600 h-32">
                                @foreach($teachers as $teacher)
                                    <option value="{{ $teacher->id }}" {{ (collect(old('teachers'))->contains($teacher->id)) ? 'selected' : '' }}>{{ $teacher->full_name }} ({{ $teacher->subject }})</option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            ...
                            <select name="students[]" multiple class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 dark:bg-gray-900 dark:text-white dark:border-gray-600 h-48">
                                @foreach($students as $student)
                                    <option value="{{ $student->id }}" {{ (collect(old('students'))->contains($student->id)) ? 'selected' : '' }}>{{ $student->full_name }}</option>
                                @endforeach
                            </select>
                            <p class="text-xs text-gray-500 mt-1">Hold Ctrl (Windows) or Command (Mac) to select multiple.</p>
                        </div>
